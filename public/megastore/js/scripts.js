...
function showNotification() {
    if ($('.cart-notification-header').is(':hidden')) {
        $('.cart-notification-header').css({ display: 'block', opacity: 0 }).animate({ opacity: 1 }, 500);
        $('.cart-notification-header').focus();
    } else {
        $('.cart-notification-header').animate({ opacity: 0 }, 500, 'swing', function() {
            $('.cart-notification-header').css({ display: "none" });
        });
    }
}
...
function showMenuHomeV3() {
    ...
    if ($(window).width() <= 980 || check_1 == 0) {
        if (!check) {
            $('.menu_more_header').css({ top: box_offset.top + box_h, left: box_offset.left, display: "block", height: 'auto' }).animate({ height: auto }, 500);
        } else {
...
function changeTabsAuth(num) {
    $('.title-tabs-auth-detail').removeClass('active-title-tabs');
